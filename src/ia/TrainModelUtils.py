...
from tensorflow.python.keras.layers import UpSampling2D, Lambda, MaxPooling2D
import numpy as np
...
class TrainModelUtils:
    def __init__(self):
        ...
        def pre_training_process_2(image):
            ...
            image = image / 255
            print("SHAPEE", image.shape)
            return image

        def create_model():
            '''model = Sequential()
            model.add(Convolution2D(24, (3, 3), (2, 2), input_shape=(64, 64, 1), activation='elu'))
            ...
            n_nodes = 500
            model = Sequential()
            model.add((Convolution2D(filters_n, size_filter, input_shape=(64, 64, 1), activation='relu')))
